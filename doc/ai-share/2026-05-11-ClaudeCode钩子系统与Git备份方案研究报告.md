...
| hooks 配置位置 | 以为在 hooks/hooks.json | Claude Code 实际读取 settings.json 中的 hooks 字段 | 修正配置位置 |
...
| 配置文件位置要确认 | Claude Code 实际读取 settings.json 而非 hooks/hooks.json | 配置前先验证 |
