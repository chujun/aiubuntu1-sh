...
经过逐层排查，定位到根因并非虚拟机配置文件（`.vmdk` / `.vmx`）本身，而是 **VMware 安装损坏**——具体为 VMAuthdService（VMware Authorization Daemon）无法正常启动 `vmware-vmx` 子进程，导致所有虚拟机的开机操作均被卡住。通过完整的清理后重新安装，问题得以解决。
...
*文档生成时间：2026-04-29 | 由 MiniMax-M2.7-highspeed 辅助生成*
