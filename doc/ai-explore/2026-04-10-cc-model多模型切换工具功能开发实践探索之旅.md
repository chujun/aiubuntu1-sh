...
```mermaid
graph TD<br/>  A[cc-model.sh<br/>Bash Shell 脚本] --> B[Anthropic API<br/>官方端点]<br/>  A --> C[OpenRouter API<br/>兼容 Anthropic SDK]<br/>  A --> D[MiniMax API<br/>国内直连]<br/>  A --> E[Python3<br/>JSON 解析/格式化]<br/>  A --> F[~/.claude/settings.json<br/>Claude Code 配置]<br/>  A --> G[~/.config/cc-model/<br/>tokens + backups]<br/>  H[cc-model-test.sh<br/>Bash 单元测试] --> A<br/>  H --> I[mktemp 沙盒隔离]
```
...
```mermaid
pie title Claude Code 工具调用分布（估算）<br/>  "Bash" : 35<br/>  "Edit" : 28<br/>  "Read" : 12<br/>  "Write" : 8<br/>  "Glob" : 3<br/>  "Grep" : 2<br/>  "Skill" : 1
```
...
```mermaid
flowchart TD<br/>  A[会话开始<br/>解读 cc-model.sh] --> B[功能扩展:<br/>添加 minimax-cn 国内支持]<br/>  B --> C[备份机制:<br/>统一存入 backups/ 目录]<br/>  C --> D[restore 命令:<br/>恢复 settings.json]<br/>  D --> E[单元测试:<br/>cc-model-test.sh]<br/>  E --> F{59 个用例<br/>全部通过?}<br/>  F -->|3 轮调试| G[✅ 59/59 通过]<br/>  G --> H[Shell 补全:<br/>bash + zsh]<br/>  H --> I[安装调试:<br/>修复 ~/.bashrc 坏内容]<br/>  I --> J[✅ 补全正常工作]
```
...
```mermaid
flowchart LR<br/>  A[用户输入: minimax-cn] --> B[MODEL_PROVIDER<br/>minimax-cn → minimax]<br/>  B --> C[MODEL_ID<br/>minimax-cn → MiniMax-Text-01]<br/>  C --> D[PROVIDER_BASE_URL<br/>minimax → api.minimax.chat/v1]<br/>  D --> E[write_settings<br/>注入 ANTHROPIC_BASE_URL]
```

### 5.3 备份机制设计

```mermaid
sequenceDiagram<br/>  participant U as 用户<br/>  participant S as cc-model switch<br/>  participant B as backup_file()<br/>  participant D as backups/ 目录<br/>  U->>S: cc-model switch gemini<br/>  S->>B: backup_file(settings.json)<br/>  B->>B: 检查文件是否存在<br/>  B->>D: cp → settings.json.bak.20260410_040000<br/>  B->>D: ls -t | tail -n +6 → 清理旧备份<br/>  S->>S: python3 合并 JSON<br/>  S->>S: write_settings 写入<br/>  U->>S: cc-model restore 2<br/>  S->>B: backup_file(当前 settings)<br/>  S->>S: cp 备份 → settings.json<br/>  S-->>U: 已恢复 ← settings.json.bak.XXX
```
...
```mermaid
flowchart TD<br/>  A[cc-model-test.sh] --> B[全局 source cc-model.sh<br/>避免 declare -A 局部化]<br/>  B --> C[覆盖 error 为 return 1<br/>防止 exit 终止测试进程]<br/>  C --> D[_run_check 子 shell<br/>set -e + error return 1]<br/>  D --> E[if 外部捕获 $?<br/>绕过 POSIX if 条件抑制 set -e]<br/>  E --> F[8 个测试模块<br/>59 个用例]<br/>  F --> G[✅ 59/59 全通过]
```
...
```mermaid
mindmap<br/>  root((cc-model<br/>开发经验))<br/>    Bash 测试方法论<br/>      source 在全局作用域<br/>      declare -A 局部变量陷阱<br/>      子 shell + set -e 组合<br/>      if 条件抑制 set -e<br/>      $? 在 if 外捕获<br/>    AI 辅助调试<br/>      bash -x 追踪执行路径<br/>      最小化复现脚本<br/>      逐层缩小问题范围<br/>    安全开发习惯<br/>      先备份再操作<br/>      操作前确认用户<br/>      沙盒隔离测试<br/>    工具设计原则<br/>      动态补全 > 静态列表<br/>      source 守卫支持测试<br/>      备份自动化
```
