...
# -*- coding: utf-8 -*-
"""
为 Packer 构建 Ubuntu 24 Server 写入 VMware NAT DHCP 静态保留。

用途：
  让固定 MAC 00:50:56:24:15:01 每次都拿到 192.168.40.150，
  避免 Packer SSH 连接固定地址时，VMware DHCP 实际分配其他 IP。

安全措施：
  1. 修改前自动备份 C:\\ProgramData\\VMware\\vmnetdhcp.conf。
  2. 重复执行时会先删除旧的 Packer reservation block，再写入新配置。
...
BEGIN_MARKER = "# BEGIN PACKER DHCP RESERVATION"
END_MARKER = "# END PACKER DHCP RESERVATION"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="写入 VMware NAT DHCP 静态保留，供 Packer 构建 Ubuntu 镜像使用。"
    )
