...
- [x] 3.1 在控制节点 VM 中安装 Ansible（pip3 install ansible ansible-core）
- [x] 3.2 安装 ansible-galaxy collections：`community.general`
- [x] 3.3 创建 `ansible/` 项目目录结构
- [x] 3.4 编写 `ansible.cfg` 配置文件
- [x] 3.5 创建 Git 仓库并初始化（用于配置版本化）
- [x] 3.6 验证 Ansible 安装：`ansible --version`
...
- [ ] 5.6 创建 `roles/common/hostname/tasks/main.yml` 主任务文件（修改 /etc/hostname、/etc/hosts）
