...
### 需求：Cloud-Init 配置系统盘分区

系统必须通过 Cloud-Init autoinstall 显式配置 Ubuntu Server 基础镜像的系统盘分区，满足 40GB 虚拟磁盘的基础容量规划。

#### 场景：40GB 磁盘分区布局
- **当** Cloud-Init 使用 40GB 虚拟磁盘安装 Ubuntu Server 时
- **则** 创建 EFI 分区并挂载到 `/boot/efi`
- **且** 创建 `/boot` 分区
- **且** 创建 LVM 卷组 `ubuntu-vg`
- **且** 创建 20GB 根逻辑卷并挂载到 `/`
- **且** 创建数据逻辑卷并将剩余 VG 空间挂载到 `/data`

#### 场景：数据分区使用剩余空间
- **当** EFI、`/boot`、根逻辑卷和 LVM 元数据占用空间后
- **则** `/data` 不硬编码为 25GB
- **且** `/data` 使用 LVM 剩余空间，避免 40GB 磁盘因额外引导分区和元数据导致容量超配
