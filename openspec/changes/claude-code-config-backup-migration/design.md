...
```json
{
  "hooks": {
    "SessionEnd": [
      {
        "type": "command",
        "command": "~/.claude/my-scripts/my-backup.sh"
      }
    ]
  }
}
...
if [ $? -ne 0 ]; then
    # 冲突无法自动解决
    echo "[$(date)] Sync FAILED: unresolved conflicts - manual intervention required" >> ~/.claude/my-backups/my-sync.log
    exit 1
fi
git push origin main
```
...
~/.claude/
├── my-scripts/                    # 脚本目录
│   ├── my-init-backup.sh         # 初始化脚本（首次使用）
│   ├── my-backup.sh              # 备份脚本（每次退出触发）
│   └── my-restore-backup.sh      # 恢复脚本（迁移到新系统）
├── my-backups/                   # 备份数据目录
│   └── my-sync.log              # 同步日志
├── hooks/
│   └── hooks.json               # [修改] 添加 SessionEnd 钩子
├── .git/                        # [新增] git 仓库
└── ...                         # 其他现有文件和目录
```
...
1. 用户运行初始化脚本 `my-init-backup.sh`
2. 脚本检查 `gh` CLI 是否安装并登录
3. 提示用户输入仓库名称（默认 `claude-config-backup`）
4. 创建 GitHub 私有仓库
5. 在 `~/.claude/` 初始化 git 仓库
6. 添加远程仓库并执行首次推送
7. 配置 SessionEnd 钩子触发 `my-backup.sh`

### 恢复（迁移到新系统）

1. 用户在新系统上运行恢复脚本 `my-restore-backup.sh`
...
在 `~/.claude/` 目录下创建 `.gitignore` 文件，用于忽略运行时产生的临时文件。
...
| `backups/` | Claude Code 自动创建的备份 |
| `file-history/` | 文件变更历史 |
| `mcp-needs-auth-cache.json` | MCP 认证缓存 |
| `paste-cache/` | 粘贴缓存 |
| `plugins/cache/` | 插件缓存 |
| `my-backups/my-sync.log` | 备份脚本产生的同步日志 |
