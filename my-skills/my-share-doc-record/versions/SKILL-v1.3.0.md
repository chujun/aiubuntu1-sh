...
> 脚本内容见 [SCRIPTS/collect_metadata.sh](./SCRIPTS/collect_metadata.sh)，包含：
> - 当前日期、项目路径、GitHub 地址
> - 当前分支与最近提交
> - 并行技术栈检测（12 种标记文件）
> - MCP 服务动态读取
...
> 脚本内容见 [SCRIPTS/validate_mermaid.sh](./SCRIPTS/validate_mermaid.sh)，包含：
> - mermaid-cli 优先验证 + 浏览器可用性检测
> - 浏览器不可用时自动切换 Python 静态检查（含未闭合括号检查）
> - 支持 `<br/>` 检测、关键字与节点同行检测、转义字符检测
...
```bash
bash SCRIPTS/auto_commit.sh <统一文档项目路径> <文档标题>
```

> 脚本内容见 [SCRIPTS/auto_commit.sh](./SCRIPTS/auto_commit.sh)，包含：
> - 版本元数据自动更新（VERSIONS.json、CHANGELOG.md）
> - 仅添加 doc/ai-share/ 目录
> - 自动提交并推送到 GitHub
