...
description: 根据当前会话内容，生成「XX应用XXX实践探索之旅」风格的 AI 学习文档，原始 Markdown 存放到统一文档项目 doc/ai-explore/，当前项目放跳转 HTML，并自动 git push 到远端。
origin: local
version: "1.5.1"
updated: "2026-04-11"
---
...
**链接优先级：**
1. **GitHub 链接**（优先）：如果统一文档项目有 GitHub 远端，使用 `https://github.com/{owner}/{repo}/blob/{branch}/doc/ai-explore/{文件名}.md` 格式
2. **本地文件链接**（降级）：如果没有 GitHub 远端，使用 `file://{绝对路径}` 格式

**统一文档项目路径的记忆机制：**
- 首次执行时询问用户统一文档项目路径，保存到 memory 中
- 后续执行时直接从 memory 读取，不再反复确认
