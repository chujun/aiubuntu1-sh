...
- 第六章"测试结果"：**仅当会话中实际运行了测试时生成**；否则替换为"六、关键决策记录"，记录会话中的重要技术决策
- 第七章"难点与挑战"：**必须包含**，记录初次判断错误、工具失败、走弯路等真实过程
- 其余章节：固定包含
